...
void getPos(sf::Vector2f &pos, sf::Vector2f mapSize) {
  ...
  } while ((!(pos.x > 10 && pos.x < mapSize.x - 10) ||
            !(pos.y > 10 && pos.y < mapSize.y - 10)));
}

void checkCollisions(std::vector<FoodObj> &foods, Player &player,
                     sf::Vector2f foodpos, int &numScore, sf::Vector2f mapSize,
                     bool &threadsClosed) {
  while (!threadsClosed) {
    for (auto &&food : foods) {
      if (player.getPlayerBlob().getGlobalBounds().intersects(
              food.getFoodBlob().getGlobalBounds())) {
        ...
        player.setPlayerSize(sf::Vector2f(player.getPlayerSize().x + 1.0f,
                                          player.getPlayerSize().y + 1.0f),
                             500);
...
void checkFoodsToDraw(std::vector<FoodObj> &foods,
                      std::vector<FoodObj> &foodsToDraw, sf::View &player_view,
                      bool &threadsClosed) {
  while (!threadsClosed) {
    std::vector<FoodObj> temp;
    for (auto &&food : foods) {
      if (((food.getFoodBlob().getPosition().x <
            player_view.getCenter().x + 5 + player_view.getSize().x / 2) &&
           (food.getFoodBlob().getPosition().x >
            player_view.getCenter().x - 5 - player_view.getSize().x / 2)) &&
          ((food.getFoodBlob().getPosition().y <
            player_view.getCenter().y + 5 + player_view.getSize().y / 2) &&
           (food.getFoodBlob().getPosition().y >
            player_view.getCenter().y - 5 - player_view.getSize().y / 2)))
        temp.push_back(food);
    }
...
int main() {
  ...
  Player player(sf::Vector2f(25.0f, 25.0f), sf::Vector2f(H / 2, W / 2),
                sf::Color::Red, texture1);
  ...
  sf::View player_view(
      sf::FloatRect(0, 0, window.getSize().x - 224, window.getSize().y - 224));
  ...
  for (int i = 0; i < 1000; i++) {
    // getPos foodpos, w, h
    getPos(foodpos, mapSize);
    FoodObj food(sf::CircleShape(10.0f),
                 sf::Color(rand() % 255, rand() % 255, rand() % 255), foodpos);
  ...
  std::thread threadCollision(checkCollisions, std::ref(foods),
                              std::ref(player), foodpos, std::ref(numScore),
                              mapSize, std::ref(threadClosed));
  std::thread threadVisFood(checkFoodsToDraw, std::ref(foods),
                            std::ref(foodsToDraw), std::ref(player_view),
                            std::ref(threadClosed));
  ...
  while (window.isOpen()) {
    ...
    player_view.setCenter(
        player.getPlayerPos().x + player.getPlayerSize().x / 2,
        player.getPlayerPos().y + player.getPlayerSize().y / 2);
    window.setView(player_view);
    scoreText.setPosition(
        player_view.getCenter().x - player_view.getSize().x / 2,
        player_view.getCenter().y - player_view.getSize().y / 2);
    numText.setPosition(
        player_view.getCenter().x + 125 - player_view.getSize().x / 2,
        player_view.getCenter().y + 5 - player_view.getSize().y / 2);
